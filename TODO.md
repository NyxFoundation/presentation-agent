...
- [x] 文字が被って醜いので配置を直す。(ラベルをドーナツ外周の空き領域に再配置)
- [x] out-of-scope (監査対象外) 部分に対して「暗号でカバーできる」ことが伝わりづらい —
      赤い 87% の領域を暗号でカバーできる、が図から直接読めるようにする。
      (赤 2 スライスを緑の外周アークで束ね「暗号で検証可能にできる領域」ラベルを付けた)
...
- [x] 機関マネーが入っているのがグラフから分かりづらい。2025 後半〜2026/07 の monthly の
      bar グラフにして、全体のうち Hyperliquid / Lighter 等のサービスの内訳も織り込む。
      (DefiLlama API の月次実データ。機関系シェア 6% → 18% が bar の色で読める)
- [x] 2026/09 以降の未来に対して爆伸びしているのも見せて、
      「このくらい爆伸びさせるには？？？」をグラフだけで見せる。
      (2026/10-2027/4 のゴースト bar で機関系 10 倍シナリオ + amber ？？？)
