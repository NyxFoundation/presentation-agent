...
- [x] 近年の TVL 推移を全ブロックチェーンで出し、その中に機関系サービス
...
- [x] p17 (SL22) の ZK Bridge は、この機関マネーナラティブの「例えばこういう仕組み」の
      一例としてここへ移動する (slides.md の include 順を変更)。
      (figures/tvl_institutional.py — DefiLlama 実データ + 機関系バー + 10 倍の問い +
      プライバシー / コンプライアンスの 2 カード。SL22 はタイトルを「例: ZK Bridge」に変更)
