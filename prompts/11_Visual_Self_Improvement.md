---
Description: One iteration of an externalized, visually-grounded self-improvement loop for a Slidev deck. A shell loop (scripts/refine.sh) renders the deck to per-slide PNGs, then invokes this prompt; you inspect every rendered slide, score the 8-axis executive rubric, apply surgical edits to slides/SL*.md, and emit a JSON review. The loop re-renders and re-invokes you until the rubric converges. Adapted from the visually-3d recursive self-improvement methodology.
...
Improve the deck until it passes **two acceptance tests**:
...
2. **The delivery test — "a senior leader could present it tomorrow."**
...
Neither test is "looks decorated." Both are *legibility* and *persuasion*.
...
1. **Critique (harsh examiner).** Read every slide PNG. Walk the rubric axis
   ...
   echo the hook, missing brand voice, fabricated-looking numbers.
...
## The 8-axis executive rubric

Score each axis 1.0–5.0. Target: average ≥ 4.5 and no axis < 4.5.

| Axis | JSON key | Full marks means |
|---|---|---|
| Clarity | `clarity` | Non-experts follow with no footnotes; one idea per slide. |
...
`average` is the mean of the eight scores. `changelog` is what you edited this
